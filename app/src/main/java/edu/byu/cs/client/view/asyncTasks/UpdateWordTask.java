...
public class UpdateWordTask extends AsyncTask<NewWordRequest, Void, NewWordResponse> {
    ...
    public interface Observer {
        void updateWord(NewWordResponse response);
        void handleException(Exception exception);
    }

    public UpdateWordTask(DictionaryPresenter presenter, Observer observer) {
    ...
    protected NewWordResponse doInBackground(NewWordRequest... requests) {
        ...
        try {
            response = presenter.updateWord(requests[0]);
        }
    ...
    protected void onPostExecute(NewWordResponse response) {
        ...
        else observer.updateWord(response);
    }
}