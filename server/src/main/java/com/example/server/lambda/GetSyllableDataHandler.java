...
import com.example.server.service.SyllableService;
...
public class GetSyllableDataHandler implements RequestHandler<GetLanguageDataRequest, GetLanguageDataResponse> {
    @Override
    public GetLanguageDataResponse handleRequest(GetLanguageDataRequest input, Context context) {
        SyllableService syllableService = new SyllableService();
        GetLanguageDataResponse response = null;
        try {
            response = syllableService.getSyllableData(input);
