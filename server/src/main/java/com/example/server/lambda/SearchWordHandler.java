...
public class SearchWordHandler implements RequestHandler<SearchWordRequest, DictionaryPageResponse> {

    @Override
    public DictionaryPageResponse handleRequest(SearchWordRequest input, Context context) {
        DictionaryService dictionaryService = new DictionaryService();
        DictionaryPageResponse response = null;
        try {
            response = dictionaryService.searchWord(input);
