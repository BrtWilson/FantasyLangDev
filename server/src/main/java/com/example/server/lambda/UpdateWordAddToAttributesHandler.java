...
public class UpdateWordAddToAttributesHandler implements RequestHandler<NewWordRequest, NewWordResponse> {
    @Override
    public NewWordResponse handleRequest(NewWordRequest input, Context context) {
        DictionaryService dictionaryService = new DictionaryService();
        NewWordResponse response = null;
        try {
            response = dictionaryService.updateWordAddToAttributes(input);
